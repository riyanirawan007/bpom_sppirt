...
$conn = mysqli_connect('localhost','root','','db_sppirt_new');
$q = mysqli_query($conn, "SELECT * FROM tabel_konfig_email WHERE id = 1");
$data = mysqli_fetch_array($q);

$config['useragent']        = 'PHPMailer';              // Mail engine switcher: 'CodeIgniter' or 'PHPMailer'
$config['protocol']         = $data['protocol'];                   // 'mail', 'sendmail', or 'smtp'
$config['mailpath']         = '/usr/sbin/sendmail';
$config['smtp_host']        = $data['host'];
$config['smtp_auth']        = $data['auth'];                     // Whether to use SMTP authentication, boolean TRUE/FALSE. If this option is omited or if it is NULL, then SMTP authentication is used when both $config['smtp_user'] and $config['smtp_pass'] are non-empty strings.
$config['smtp_user']        = $data['user'];
$config['smtp_pass']        = $data['pass'];
$config['smtp_port']        = $data['port'];
$config['smtp_timeout']     = $data['timeout'];                       // (in seconds)
$config['smtp_crypto']      = $data['crypto'];                    // '' or 'tls' or 'ssl'
