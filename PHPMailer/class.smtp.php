...
class SMTP
{
    ...
    public function authenticate(
        ...
        // SMTP extensions are available. Let's try to find a proper authentication method
            ...
            self::edebug(
                'Auth methods available on the server: ' . implode(',',  (array)$this->server_caps['AUTH']),
                self::DEBUG_LOWLEVEL
            );

            if (empty($authtype)) {
                foreach (array('LOGIN', 'CRAM-MD5', 'NTLM', 'PLAIN') as $method) {
                    if (in_array($method, (array)$this->server_caps['AUTH'])) {
                        $authtype = $method;
                        break;
                    }
                }
            ...
            if (!in_array($authtype, (array)$this->server_caps['AUTH'])) {
